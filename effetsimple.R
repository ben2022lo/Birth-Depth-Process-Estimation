...
plot(X[[1]]@times, X[[1]]@states, type = "l", xlab = "Time", ylab = "States", main = "BDMC States over Time")
...
SGD <- function(X_discret, learning_rate = 0.01, epochs = 100, batch_size = 1, N_latent_sim = 10, k_max = 50) {
  ...
  Lamb <- runif(1); mu <- runif(1); nu = runif(1, 0, 3)
  ...
  for (epoch in 1:epochs) {
    ...
    for (i in seq(1, N, by = batch_size)) {
      ...
      X_batch_cond <- sim.condBD(N=N_latent_sim, bd.PO=X_batch_discret[[1]], L=Lamb, m=mu, nu=nu) # latent variable x
      ...
      gradient_Lamb <- sum(U_k * (k_vector / (k_vector * Lamb + nu)) - k_vector * T_k)
      gradient_mu <- sum(D_k / mu - k_vector * T_k)
      gradient_nu <- sum(U_k / (k_vector * Lamb + nu) - T_k)
...
parameters <- SGD(X_discret, learning_rate = 0.01, epochs = 10, batch_size = 1, N_latent_sim = 3)
